...
namespace RotaryHeart.Lib.PhysicsExtension
{
    ...
    public static class Physics
    {
        ...
        {
            private static System.Collections.Generic.List<Collider> COLLIDERS = new System.Collections.Generic.List<Collider>();
            ...
            #region Vector3
            public static bool SightConeCast(Vector3 origin, Vector3 direction, float length, float angle, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCast(origin, direction, length, angle, LAYER_MASK, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static bool SightConeCast(Vector3 origin, Vector3 direction, float length, float angle, LayerMask mask, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCast(origin, direction, length, angle, mask, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static bool SightConeCast(Vector3 origin, Vector3 direction, float length, float angle, LayerMask mask, QueryTriggerInteraction queryTriggerInteraction, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                direction.Normalize();
                if (preview)
                {
                    DebugExtensions.DebugConeSight(origin, direction, length, noHitColor ?? Color.red, angle, drawDuration);
                }

                foreach (var overlapping in OverlapSphere(origin, 10, mask, queryTriggerInteraction))
                {
                    if (ignoreTransforms != null && ignoreTransforms.Contains(overlapping.transform))
                        continue;

                    Vector3 heading = (overlapping.transform.position - origin).normalized;
                    float dot = Vector3.Dot(direction, heading);

                    if (dot >= Mathf.Cos(angle * Mathf.Deg2Rad))
                    {
                        if (preview)
                        {
                            DebugExtensions.DebugPoint(overlapping.transform.position, hitColor ?? Color.green, 0.5f, drawDuration);
                            Debug.DrawLine(origin, overlapping.transform.position, hitColor ?? Color.green, drawDuration);
                        }
                        return true;
                    }
                }
            ...
            #region Ray
            public static bool SightConeCast(Ray ray, float length, float angle, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCast(ray, length, angle, LAYER_MASK, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static bool SightConeCast(Ray ray, float length, float angle, LayerMask mask, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCast(ray, length, angle, mask, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static bool SightConeCast(Ray ray, float length, float angle, LayerMask mask, QueryTriggerInteraction queryTriggerInteraction, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                ray.direction.Normalize();
                if (preview)
                {
                    DebugExtensions.DebugConeSight(ray.origin, ray.direction, length, noHitColor ?? Color.red, angle, drawDuration);
                }

                foreach (var overlapping in OverlapSphere(ray.origin, 10, mask, queryTriggerInteraction))
                {
                    if (ignoreTransforms != null && ignoreTransforms.Contains(overlapping.transform))
                        continue;

                    Vector3 heading = (overlapping.transform.position - ray.origin).normalized;
                    float dot = Vector3.Dot(ray.direction, heading);
                    ...
                    if (dot >= Mathf.Cos(angle * Mathf.Deg2Rad))
                    {
                        ...
                        {
                            DebugExtensions.DebugPoint(overlapping.transform.position, hitColor ?? Color.green, 0.5f, drawDuration);
                            Debug.DrawLine(ray.origin, overlapping.transform.position, hitColor ?? Color.green, drawDuration);
                        }
                        return true;
                    }
                }
            ...
            #region Vector3
            public static Collider[] SightConeCastAll(Vector3 origin, Vector3 direction, float length, float angle, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastAll(origin, direction, length, angle, LAYER_MASK, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static Collider[] SightConeCastAll(Vector3 origin, Vector3 direction, float length, float angle, LayerMask mask, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastAll(origin, direction, length, angle, mask, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static Collider[] SightConeCastAll(Vector3 origin, Vector3 direction, float length, float angle, LayerMask mask, QueryTriggerInteraction queryTriggerInteraction, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                direction.Normalize();
                COLLIDERS.Clear();
                ...
                foreach (var overlapping in OverlapSphere(origin, length, mask, queryTriggerInteraction))
                {
                    if (ignoreTransforms != null && ignoreTransforms.Contains(overlapping.transform))
                        continue;
            ...
            #region Ray
            public static Collider[] SightConeCastAll(Ray ray, float length, float angle, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastAll(ray, length, angle, LAYER_MASK, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static Collider[] SightConeCastAll(Ray ray, float length, float angle, LayerMask mask, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastAll(ray, length, angle, mask, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static Collider[] SightConeCastAll(Ray ray, float length, float angle, LayerMask mask, QueryTriggerInteraction queryTriggerInteraction, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                ray.direction.Normalize();
                COLLIDERS.Clear();
            ...
            #region Vector3
            public static int SightConeCastNonAlloc(Vector3 origin, Vector3 direction, Collider[] results, float length, float angle, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastNonAlloc(origin, direction, results, length, angle, LAYER_MASK, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static int SightConeCastNonAlloc(Vector3 origin, Vector3 direction, Collider[] results, float length, float angle, LayerMask mask, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastNonAlloc(origin, direction, results, length, angle, mask, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static int SightConeCastNonAlloc(Vector3 origin, Vector3 direction, Collider[] results, float length, float angle, LayerMask mask, QueryTriggerInteraction queryTriggerInteraction, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                direction.Normalize();
                COLLIDERS.Clear();
                ...
                foreach (var overlapping in OverlapSphere(origin, 10, mask, queryTriggerInteraction))
                {
                    ...
                    if (dot >= Mathf.Cos(angle * Mathf.Deg2Rad))
                    {
                        ...
                        {
                            results[resultsIndex] = overlapping;
                            resultsIndex++;
                        }
                    }
                }
            ...
            #region Ray
            public static int SightConeCastNonAlloc(Ray ray, Collider[] results, float length, float angle, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastNonAlloc(ray, results, length, angle, LAYER_MASK, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static int SightConeCastNonAlloc(Ray ray, Collider[] results, float length, float angle, LayerMask mask, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                return SightConeCastNonAlloc(ray, results, length, angle, mask, QUERY_TRIGGER_INTERACTION, ignoreTransforms, preview, drawDuration, hitColor, noHitColor);
            }

            public static int SightConeCastNonAlloc(Ray ray, Collider[] results, float length, float angle, LayerMask mask, QueryTriggerInteraction queryTriggerInteraction, System.Collections.Generic.List<Transform> ignoreTransforms = null, bool preview = false, float drawDuration = 0, Color? hitColor = null, Color? noHitColor = null)
            {
                ray.direction.Normalize();
                COLLIDERS.Clear();
